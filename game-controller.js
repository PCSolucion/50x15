        ...
        if (this.uiController && this.uiController.elements.moneyLevels) {
            this.uiController.elements.moneyLevels.forEach(level => {
                ...
                if (levelNumber && this.config.prizes[levelNumber]) {
                    const iconUrl = (lvl => {
                        // Icono especial para niveles seguros 5 y 10
                        if (lvl === 5 || lvl === 10) {
                            return 'https://res.cloudinary.com/pcsolucion/image/upload/v1756533278/gempoucht1_d7u0c0.png';
                        }
                        if (lvl >= 11 && lvl <= 15) {
                            return 'https://res.cloudinary.com/pcsolucion/image/upload/v1756532957/coinbundle_r1cf82.png';
                        }
                        if (lvl >= 6 && lvl <= 10) {
                            return 'https://res.cloudinary.com/pcsolucion/image/upload/v1756532956/coinpile_q2bwi9.png';
                        }
                        return 'https://res.cloudinary.com/pcsolucion/image/upload/v1756532957/coin-heap_pzbj1o.png';
                    })(levelNumber);

                    level.innerHTML = `<img src="${iconUrl}" alt="premio" class="prize-icon"> ${this.config.prizes[levelNumber].toLocaleString()}`;
                }
            });
        }
    }
